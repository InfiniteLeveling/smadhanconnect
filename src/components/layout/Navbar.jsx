        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-2.5">
              ...
              {/* Persona Profile Card & Dropdown Menu */}
              {profile ? (
                <div className="relative">
                  <button
                    onClick={() => setShowUserMenu(!showUserMenu)}
                    className="hidden sm:flex items-center gap-2.5 pl-3 border-l border-slate-200 hover:opacity-85 transition-opacity cursor-pointer text-left"
                  >
                    <img
                  ...
                  {showUserMenu && (
                    <div 
                      ...
                      className="absolute right-0 mt-2 w-64 bg-white/95 backdrop-blur-xl rounded-2xl shadow-xl border border-slate-200/90 p-3 space-y-2 z-50 animate-in fade-in zoom-in-95"
                    >
                      <div className="pb-2 border-b border-slate-100 px-1">
                        <p className="text-xs font-bold text-slate-900 truncate">{profile.full_name}</p>
                        <p className="text-[11px] text-slate-400 truncate">{profile.email || `${profile.role.toLowerCase()}@jharkhand.gov.in`}</p>
                        <span className="inline-block mt-1 text-[10px] font-mono font-bold uppercase text-brand-700 bg-brand-50 px-2 py-0.5 rounded-md border border-brand-200/60">
                          Role: {profile.role}
                        </span>
                      </div>
                      ...
                      <div className="space-y-1">
                        <Link
                          ...
                          className="flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-semibold text-slate-700 hover:bg-slate-100 hover:text-brand-700 transition-colors"
                        >
            ...
            <div className="pt-3 border-t border-slate-100 flex flex-col gap-2">
              {profile ? (
                <div className="p-3 bg-slate-50 border border-slate-200/80 rounded-2xl flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] font-mono text-brand-600 font-bold uppercase">{profile.role}</p>
                    </div>
                  </div>
                  <button
                    onClick={() => {
                ...
                <Link to="/login" onClick={() => setMobileMenuOpen(false)}>
                  <button className="w-full py-2.5 rounded-xl text-xs font-bold text-slate-700 bg-slate-100 hover:bg-slate-200">
                    Sign In
                  </button>
                </Link>
              )}
