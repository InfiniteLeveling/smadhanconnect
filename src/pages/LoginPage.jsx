...
const ECOSYSTEM_ROLES = [
  {
    id: 'CITIZEN',
    name: 'Citizen',
    description: 'Report and track civic challenges',
    icon: Users,
  ...
  },
  {
    id: 'UNIVERSITY',
    name: 'University',
    description: 'Collaborate on civic R&D',
    icon: GraduationCap,
  ...
  },
  {
    id: 'INDUSTRY',
    name: 'Industry / CSR',
    description: 'Fund & scale civic solutions',
    icon: Building2,
  ...
  },
  {
    id: 'STUDENT',
    name: 'Innovator',
    description: 'Propose technical solutions',
    icon: Rocket,
...
export const LoginPage = () => {
  ...
  return (
    <div className="relative min-h-screen bg-slate-50 text-slate-900 flex flex-col justify-between overflow-x-hidden select-none">
      
      {/* 3D Animated Background Canvas (Subtle Civic Mesh) */}
      <CivicNetworkCanvas className="h-full min-h-screen -z-0 opacity-80" />

      {/* Decorative Gradient Ray Overlay */}
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-7xl h-96 bg-gradient-to-b from-brand-100/35 via-emerald-50/15 to-transparent pointer-events-none -z-10" />

      {/* ========================================================================= */}
      {/* TOP BRANDING BAR & BACK BUTTON                                            */}
      {/* ========================================================================= */}
      <header className="relative z-20 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 flex items-center justify-between">
        ...
        <Link
          to="/"
          className="group inline-flex items-center gap-2 px-3.5 py-1.5 rounded-xl text-xs font-bold text-slate-600 bg-white/80 backdrop-blur-md border border-slate-200/80 shadow-sm hover:text-brand-700 hover:border-brand-300 hover:bg-white transition-all duration-200"
        >
      ...
      <main className="relative z-10 flex-1 flex items-center justify-center px-4 sm:px-6 py-6 sm:py-10">
        <div className="w-full max-w-xl">
          ...
          {/* Glassmorphism Card */}
          <div className="relative bg-white/90 backdrop-blur-2xl rounded-3xl border border-slate-200/90 shadow-2xl shadow-emerald-950/10 p-6 sm:p-9 animate-float-subtle transition-all duration-300">
            ...
            <div className="text-center space-y-2 mb-6">
              ...
              <p className="text-xs sm:text-sm text-slate-500 font-medium">
                Sign in to continue to <span className="font-semibold text-slate-700">Samadhan.Connect</span>
              </p>
            </div>

            {/* Error Message Banner */}
            {errorMessage && (
              <div className="mb-5 p-3.5 rounded-2xl bg-red-50 border border-red-200 flex items-start gap-2.5 text-xs text-red-700 animate-in fade-in-50">
            ...
            {isSuccess && (
              <div className="mb-5 p-4 rounded-2xl bg-emerald-50 border border-emerald-200 flex items-center justify-center gap-2.5 text-xs text-emerald-800 font-bold animate-in zoom-in-95">
                <CheckCircle2 className="w-5 h-5 text-emerald-600 animate-bounce" />
                <span>Authenticated Successfully! Redirecting to {selectedRole.name} workspace...</span>
              </div>
            )}

            {/* =================================================================== */}
            {/* 1. ROLE SELECTOR ("Continue as")                                    */}
            {/* =================================================================== */}
            <div className="space-y-2.5 mb-6">
              <div className="flex items-center justify-between">
                <label className="text-xs font-bold font-display uppercase tracking-wider text-slate-600">
                  Continue as
                </label>
                <span className="text-[11px] text-brand-700 font-medium bg-brand-50 px-2 py-0.5 rounded-md border border-brand-200/60">
                  Select Persona
                </span>
              </div>

              {/* 5 Selectable Role Chips */}
                ...
                {ECOSYSTEM_ROLES.map((role) => {
                  ...
                  return (
                    <button
                      ...
                      className={`group relative p-2.5 rounded-2xl border text-left transition-all duration-200 flex items-start gap-2.5 cursor-pointer ${
                        isSelected
                          ? 'bg-brand-50/80 border-brand-500 ring-2 ring-brand-500/20 shadow-xs scale-[1.02]'
                          : 'bg-white/60 border-slate-200/80 hover:bg-white hover:border-brand-300'
                      }`}
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <p className={`text-xs font-bold truncate leading-tight ${isSelected ? 'text-brand-900' : 'text-slate-800'}`}>
                          {role.name}
                        </p>
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative rounded-2xl shadow-xs">
                  ...
                  <input
                    ...
                    className="block w-full pl-10 pr-4 py-2.5 text-xs sm:text-sm text-slate-900 bg-white/70 backdrop-blur-sm border border-slate-200 rounded-2xl placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 focus:bg-white transition-all"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    type="button"
                    onClick={() => alert('For SIH Demo: Any password works for offline simulated credentials, or reset password via your Supabase instance.')}
                ...
                <div className="relative rounded-2xl shadow-xs">
                  ...
                  <input
                    ...
                    placeholder="••••••••"
                    disabled={isLoading || isSuccess}
                    className="block w-full pl-10 pr-10 py-2.5 text-xs sm:text-sm text-slate-900 bg-white/70 backdrop-blur-sm border border-slate-200 rounded-2xl placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 focus:bg-white transition-all"
                  />
                  <button
                    type="button"
              ...
              <div className="pt-2">
                <button
                  type="submit"
                  disabled={isLoading || isSuccess}
                  className="group relative w-full flex items-center justify-center gap-2 py-3.5 px-6 rounded-2xl text-sm font-bold text-white bg-gradient-to-r from-brand-700 via-emerald-600 to-teal-600 shadow-lg shadow-brand-600/25 hover:shadow-xl hover:shadow-brand-500/35 hover:scale-[1.01] active:scale-[0.98] transition-all duration-300 cursor-pointer disabled:opacity-75 disabled:pointer-events-none overflow-hidden"
                >
            ...
            <div className="mt-6 pt-5 border-t border-slate-100 text-center space-y-3">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-2xl text-xs font-bold text-slate-700 bg-white border border-slate-200/90 shadow-2xs hover:bg-slate-50 hover:border-brand-300 hover:text-brand-800 transition-all cursor-pointer"
              >
            ...
            <div className="mt-6 pt-4 border-t border-slate-100 text-center space-y-3">
              <p className="text-xs text-slate-500 font-medium">
                ...
                <Link
                  to="/report-problem"
                  className="font-bold text-brand-700 hover:text-brand-800 hover:underline inline-flex items-center gap-0.5"
                >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl border border-slate-200 space-y-4">
            ...
            <p className="text-xs text-slate-600 leading-relaxed">
              Integration ready for <strong>MeriPehchaan</strong> & <strong>Jan Parichay SSO</strong> for verified state departmental nodal officers. Use the <em>Government / Nodal</em> persona role on this page for live simulation.
            </p>
      ...
      <footer className="relative z-10 py-4 border-t border-slate-200/80 text-center text-[11px] text-slate-400">
        <span>Samadhan.Connect • Department of IT & e-Governance, Govt. of Jharkhand</span>
      </footer>
